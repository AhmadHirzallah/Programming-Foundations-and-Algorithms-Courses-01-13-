...
void	ft_print_sum_of_2d_array_columns(int **array)
{
	short	c;

	c = -1;
	while (++c < 3)
		printf("Column %hd summation is: %d\n" , c + 1,ft_get_sum_of_2d_array_column(array, c));
}
...
int	main(void)
{
	...
	printf("______\n");
	ft_print_sum_of_2d_array_columns(p_2d_array);
	free(p_2d_array);
	return (0);
}
