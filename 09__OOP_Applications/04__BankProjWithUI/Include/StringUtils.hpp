...
class	StringUtils
{
	public:
				...
				// // Variadic function to join multiple strings with a separator
				// template <typename... Args>
				// static std::string	joinVariadic(const std::string& separator, const Args&... args)
				// {
				// 	return (joinSingle(args) + ... + (separator + joinSingle(args)));
				// }
				...
				static int	countWordsWithDelim(const std::string &str, const std::string &delim = " ")
				{
					...
					while (start < str.length() && str.substr(start, delim.length()) == delim)
				        start += delim.length();
					...
					if (start < str.length() && (str.find(delim, start) == std::string::npos))
						counter++;
					return (counter);
				}
				...
				{
					std::size_t	start_index = 0	,	end_index = str.length();
				...
				static std::string replaceWord(std::string &str, std::string &replace,
												std::string &replace_to, const std::string &delimiter = " ",
				...
				void	replaceWord(std::string replace, std::string replace_to,
									const std::string &delimiter = " ", bool match_case = true)
				{
	...
	private:
				...
				// // Converts non-string types to string (helper function)
				// template <typename T>
				// static std::string	joinSingle(const T& value)
				// {
				// 	return (std::to_string(value));  // Convert to string if not already
				// }

				// // String specialization for joinSingle
				// static std::string	joinSingle(const std::string& value)
				// {
