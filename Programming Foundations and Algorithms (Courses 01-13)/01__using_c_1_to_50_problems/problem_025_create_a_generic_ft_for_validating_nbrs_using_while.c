...
void ft_read_age_until_its_valid(int *i_age ,short from, short to);
void    ft_read_age(int *i_age);
...
int main(void)
{
    ...
    if (i_age == NULL)
        return (1);
	ft_read_age_until_its_valid(i_age , from, to);
...
void ft_read_age_until_its_valid(int *i_age ,short from, short to)
{
    while (!ft_validate_nbr_in_range(*i_age, from, to))
			ft_read_age(i_age);
}
