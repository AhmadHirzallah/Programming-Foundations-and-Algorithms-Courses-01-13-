...
int	*ft_i_get_max_array_element(int i_array[100], short entered_len);
...
int	main(int argc, char **argv)
{
	...
	i_max_element = ft_malloc(1, sizeof(int));
	from = ft_malloc(1, sizeof(short));
	to = ft_malloc(1, sizeof(short));
	if (!p_i_generated_random_nbr || !from || !to || !i_max_element)
		return (2);
	...
	i_max_element = ft_i_get_max_array_element(i_array, entered_len);
...
{
	short	c;
	*i_max_element = i_array[0];

	c = 0;
	while (c < entered_len)
	{
		if (i_array[c] > *i_max_element)
			*i_max_element = i_array[c];
		c++;
	}
	return (i_max_element);
}
