...
class	CoreUtilities
{
	public:
			...
			static std::string	decryptText(std::string text, short decryption_key)
			{
			...
			static void	getArrayShuffled(int my_array[100], int len)
			{
				for (int i = 0; i < len; i++)
			...
			static std::string	NumberToText(long number) {
				...
				if (number >= 20 && number <= 99) {
					std::string arr[] = { "", "", "Twenty", "Thirty", "Forty", "Fifty",
