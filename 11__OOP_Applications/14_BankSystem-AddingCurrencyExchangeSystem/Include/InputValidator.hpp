...
class	InputValidator
{
	public:
			...
			static char	readChr(std::string display = "Please enter: ")
			{
				char	enterer_chr;
...
// Function to read an integer
			...
			static double readRangedNumberDouble(double from, double to =  std::numeric_limits<double>::max(), const std::string& promptMessage = "Please Enter the Number:  ") 
			{
				...
				while (!IsNumberBetween_Var(number, from, to))
				{
					std::cout << "Number not in range, try again (Range: " 
							<< from << " to " << to << "): ";
				
					number = readNumberDouble(""); // Skip prompt; already shown
				}
				return number;
			}
